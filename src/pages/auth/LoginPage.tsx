...
export default function LoginPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!login(username.trim(), password)) return setError('Nom d’utilisateur ou mot de passe incorrect.')
      ...
      <div className="relative w-full max-w-5xl">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          {error && <p role="alert" className="rounded-xl border border-rose-400/20 bg-rose-500/10 px-4 py-3 text-sm text-rose-300">{error}</p>}
              <button type="submit" className="btn-primary w-full py-4"><LogIn size={19} /> Connexion administrateur</button>
            </form>
          </section>
        </div>
      </div>
